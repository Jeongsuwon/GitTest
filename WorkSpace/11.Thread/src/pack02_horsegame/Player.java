...
public class Player extends Thread{
	...
	//thread.strat(); ↓ 작업은 반드시 run메소드에 구현한다.(Interface의 경우에도 run)
	
	@Override
	public void run() {
		int hereLocation = 0; //0에서부터 출발한다고 변수로 만듦.
		...
		PlayMain.rank++;
		if(PlayMain.rank==1&&PlayMain.choice==num) {
			System.out.println("정답입니다.");
		}else if(PlayMain.rank==1&&PlayMain.choice!=num) {
