...
public static void main(String[] args) {
	...
	System.out.println("1. 1세대(전화) / 2. 2세대(전화, DMB) / 3. 3세대(전화, DMB, 인터넷)");
		...
		while(true) {
			...
			if(power==false) {
				...
				if(powersc==1) {
					...
					int genCho = dao.inputInt();
					if(genCho==1) {
						dmb.function();
						while(true) {
						System.out.println("yes - 전화도 하겠다. / no - 종료하겠다. (yes / no 문자열 입력)");
